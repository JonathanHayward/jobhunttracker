;(function()
  ...
  var parse = function(raw, other)
    ...
    if ('' + raw)
      {
      if ('0123456789n[{"'.indexOf(raw[0]) === -1)
        {
        return other;
        }
      else
  ...
  var stringify = function(raw)
    {
    return JSON.stringify(raw);
    };
  var clone = function(raw)
    {
    return parse(stringify(raw));
    };
  document.getElementById('logout').click(function()
    ...
    (document.cookie =
      'password; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;');
  ...
  var todo_item_names = [
    ...
    'In Question',
    'Problems'];
  var restore = function(key, default_value, state)
    {
    ...
    var url;
    (url = '?mode=restore&internal=internal&username=' + 
      encodeURIComponent(username) + '&password=' +
      encodeURIComponent(password) + '&key=' + encodeURIComponent(key));
    jQuery.ajax(url,
      {
      'success': function(data, textStatus, jqXHR)
        {
        if (jqXHR.responseText && (jqXHR.responseText[0] === '<' ||
          jqXHR.responseText[0] === 'u'))
          {
          result = default_value;
          }
        else
          {
          var response = parse(jqXHR.responseText);
  ...
  var save = function(key, data)
    ...
    if (Modernizr.localstorage)
      {
      localStorage[key] = stringify(data);
      }
    jQuery.ajax('?mode=save',
      {
      'data':
        {
        'key': key,
  ...
  var get_todo_item = function(that)
    {
    var result = {};
    if (!that.state.hasOwnProperty('next_todo_index'))
      {
      that.state.next_todo_index = 1;
      }
    result.index = that.state.next_todo_index;
    result.key = that.state.next_todo_index;
    that.state.next_todo_index += 1;
  ...
  var Calendar = React.createClass(
    ...
    render_basic_entry: function(entry)
      ...
      result.push(React.createElement("li", null, React.createElement("input", {type: "checkbox", name: "all_day", 
        id: "all_day", defaultChecked: true}), "All day event." + ' ' +
      ...
      result.push(React.createElement("li", null, React.createElement("input", {type: "checkbox", name: "advanced", 
        ...
        "Recurring event"));
      return React.createElement("div", null, React.createElement("p", null, React.createElement("strong", null, "Add a new calendar entry:")), React.createElement("ul", null, result));
      },
            ...
            if (activities_today.length)
              ...
              for(var index = 0; index < activities_today.length;
                ...
                var minute_options = [[0, '00'],
                  ...
                  [58, '58'],
                  [59, '59']];
                var format_time = function(hour_text, minute_text)
                  {
                  if (typeof minute_text === 'undefined')
                    {
                    minute_text = '00';
                    }
                  var hour = hour_text.substr(0, hour_text.length - 2);
                  var meridian = hour_text.substr(hour_text.length - 2);
                  if (minute_text === '00')
                    {
                    return ('<span class="time"><span class="hours">' + hour +
                      '</span><span class="meridian-separator"> </span>' +
                      '<span class="meridian">' + meridian + '</span></span>');
                    }
                  else
                    {
                    return ('<span class="time"><span class="hours">' + hour +
                      '</span><span class="hour-minute-separator">:</span>' +
                      '<span class="minutes">' + minute_text +
                      '<span class="meridian-separator"> </span>' +
                      '<span class="meridian">' + meridian + '</span></span>');
                    }
                  };
                var weekday_options = ['Sunday', 'Monday', 'Tuesday',
                ...
                if (activity.all_day)
                  {
                  rendered_activities.push(React.createElement("li", {className: "all-day", 
                    dangerouslySetInnerHTML: {__html:
                    format_time(hour_options[activity.hours][1]) +
                    converter.makeHtml(activity.description)
                    .replace('<p>', '<span className="activity-description">'
                    ).replace('</p>', '</span>')}}));
                  }
                else if (activity.minutes)
                  {
                  rendered_activities.push(React.createElement("li", {className: "minutes", 
                    dangerouslySetInnerHTML: {__html:
                    format_time(hour_options[activity.hours][1],
                  ...
                  {
                  rendered_activities.push(React.createElement("li", {className: "hours", 
                    dangerouslySetInnerHTML: {__html:
                    format_time(hour_options[activity.hours][1]) +
                    weekday_options[activity.weekday] + ' ' +
                    month_options[activity.month][1] + '&nbsp;' +
                    activity.date + ', ' + activity.year + ' ' +
                    converter.makeHtml(activity.description)
                    .replace('<p>',
                    '<span className="activity-description">').replace('</p>',
                    '</span>')}}
                    ));
                  }
                }
  ...
  var Scratchpad = React.createClass(
    ...
    render: function()
      {
      var saved_state = restore('Scratchpad', '', this.setState);
      return (
      ...
      render: function()
        ...
        var display_item_details = function(label, item)
          ...
          return (
            React.createElement("td", {className: label, title: label}, 
              React.createElement("input", {onChange: that.handle_change, "data-index": html_id, 
        ...
        var display_item = function(item)
          ...
          if (item['Completed'] || item['Delete'] || item['Invisible'])
            ...
            return (
                ...
                React.createElement("td", {className: "description", dangerouslySetInnerHTML: {__html:
        ...
        return (
          React.createElement("div", {id: "Todo"}, 
            ...
            React.createElement("form", {onSubmit: that.handleSubmit}, 
              React.createElement("table", null, 
                React.createElement("tbody", null, 
                    table_rows
                ), 
                React.createElement("tfoot", null, 
                  React.createElement("textarea", {name: "todo-textarea", id: "todo-textarea", 
                    onChange: that.onChange}), React.createElement("br", null), 
                  React.createElement("button", null, 'Add activity')
                )
              )
            )
          )
    ...
    render: function() {
      ...
      var display_todo_item_checkbox = function(label, item) {
        ...
        return ( <td className={label} title={label}>
            <input onChange={that.handle_checkbox_change}
      ...
      var filtered_table_rows = table_rows.filter(function(candidate)
        {
        if (candidate['Completed'] || candidate['Delete'] ||
          candidate['Invisible'])
          {
          return false;
          }
      ...
      return ( <div id="Todo">
        ...
        <form onSubmit={that.handleSubmit}>
          <table>
              ...
              <tfoot>
                <textarea onChange={that.onChange}
                  value={that.state.text}></textarea><br />
  ...
  */
  var YouPick = React.createClass(
    ...
    getDefaultProps: function()
      ...
      return {
        ...
        interval: 100,
        repeated_text: "serving up a web application "
        };
      },
    getInitialState: function()
      {
    ...
    render: function()
      ...
      return (
        React.createElement("div", {id: "YouPick"}, 
          React.createElement("h2", null, React.createElement("em", null, "Guess what's coming next?")), 
  ...
  var update = function()
    ...
    if (previous_value !== document.getElementById('scratchpad').value)
      {
      save('Scratchpad',
        document.getElementById('scratchpad').value);
      previous_value = document.getElementById('scratchpad').value;
      }
    };
